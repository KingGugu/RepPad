    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, rec_batch, in rec_cf_data_iter:
                ...
                _, input_ids, target_pos, target_neg_list, _, seq_len = rec_batch

                # ---------- recommendation task ---------------#
                sequence_output = self.model(input_ids, seq_len)

                target_neg_list = target_neg_list.permute(1, 0, 2)
                joint_loss = 0
                for target_neg in target_neg_list:
                    rec_loss = self.cross_entropy(sequence_output, target_pos, target_neg)
                    joint_loss = joint_loss + rec_loss
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    user_ids, input_ids, target_pos, target_neg, answers, seq_len = batch
                    # recommend_output = self.model.transformer_encoder(input_ids)
                    recommend_output = self.model(input_ids, seq_len)
